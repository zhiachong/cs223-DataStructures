...
void Cheap::RebuildHeap(int root, int last)
{
    ...
    if (leftchild < msize && leftchild < last && last != 0) 
    {
        ...
        { 
           ...
           PrintTree();
           cout << endl;
           cout << "Compared " << GREEN << mitems[maxchild] << RESET;
           cout <<  " with " << GREEN << mitems[root] << RESET;
           cout << ", " << GREEN << mitems[maxchild] << RESET << " is smaller, so swapped with ";
           cout << GREEN << mitems[root] << RESET << endl;
           Continue();
