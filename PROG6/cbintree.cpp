...
// makes a tree for testing purposes.
// pre : infile is open
// post: makes a full tree of height 3, else
//       throws an exception 
...
void cbintree:: makeTree(ifstream& infile) throw (cexception)
{
   ...
   if (mroot == NULL)
   {
      ...
      infile >> item;
      mroot -> mleftptr = new Cnode (item, NULL, NULL);
      infile >> item;
      mroot -> mrightptr = new Cnode (item, NULL, NULL); 
  
      Cnode* curptr = mroot -> mleftptr;
      infile >> item;
      curptr -> mleftptr = new Cnode (item, NULL, NULL);
      infile >> item;
      curptr -> mrightptr = new  Cnode (item, NULL, NULL); 
   
      Cnode* newptr = mroot -> mrightptr;
      infile >> item;
      newptr -> mleftptr = new Cnode (item, NULL, NULL);
      infile >> item;
      newptr -> mrightptr = new Cnode (item, NULL, NULL); 
   }
}
...
void cbintree::writePretty (Cnode* treep, int level, ofstream& outfile)
{
   if (treep != NULL)
   {
      level++;
      writePretty(treep -> mrightptr, level, outfile);
      ...
      writePretty(treep -> mleftptr, level, outfile);
   }
}
